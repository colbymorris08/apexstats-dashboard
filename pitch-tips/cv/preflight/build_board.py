...
{near_html}
        ...
        <div class="callout" style="border-left: 3px solid var(--accent); background: #111a24;">
          <p>
            <strong>Analyzing High-Confidence Mechanical Variance &amp; Priority Advance Indicators:</strong><br>
            In the first row, <strong>0.884 precision</strong> on <code>glove_speed_cv</code> isolates an actionable mechanical variation in pre-release delivery tempo. Because sinkers represent 81.0% of this split, contextualizing predictive lift (+0.074) ensures advance scouts evaluate the cue alongside baseline pitch mix. In a two-pitch contrast, this high-precision signal serves as an effective directional indicator: when the cue fires, hitters anticipate the primary pitch with 88.4% certainty, and its absence provides an inverse tell that elevates secondary pitch expectations. In the third row, <code>knee_rise_duration_frac</code> achieves a <strong>+11.3% predictive lift</strong> (+0.113 over baseline) across 40 tracked deliveries, providing game planners and hitters with a measurable edge over baseline expectation that serves as a priority target for multi-angle video confirmation.
          </p>
        </div>
      </section>
